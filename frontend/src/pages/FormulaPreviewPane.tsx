...
export function FormulaPreviewPane({ result, loading, definition, canPublish, publishing, acknowledged, onAcknowledge, onPublish }: Props) {
  const issues = result?.blockingIssues || []
  const warnings = result?.warnings || []
  const statement = result?.statement as
    | { reportType?: string; groups?: { key: string; title: string; lines: { key: string; lineNo: number; name: string; indent: number; primaryAmount: string | number; comparativeAmount?: string | number }[] }[]; checks?: { key: string; name: string; passed: boolean; difference: string | number }[]; primaryColumn?: string; comparativeColumn?: string }
    | { totalLines?: number; lines?: { code: string; name: string; amount: string | number }[] }
    | undefined
  return <Card
    title="试算结果"
    extra={<Space>
      {warnings.length > 0 && <Checkbox checked={acknowledged} onChange={(event) => onAcknowledge(event.target.checked)}>确认发布带警告的版本</Checkbox>}
  ...
  >
    <Spin spinning={loading}>
      ...
      {statement && 'groups' in statement && <PreviewStatutory statement={statement} />}
...
function PreviewStatutory({ statement }: {
  statement: { groups?: { key: string; title: string; lines: { key: string; lineNo: number; name: string; indent: number; primaryAmount: string | number; comparativeAmount?: string | number }[] }[]; primaryColumn?: string; comparativeColumn?: string }
}) {
