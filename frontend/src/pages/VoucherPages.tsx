...
export function VoucherEditorPage() {
  ...
  const editable = voucherId === 'new' || (selectedPeriod?.status === 'OPEN' && !voucher.data?.sourceType)
  ...
  return <Space direction="vertical" size={16} style={{ width: '100%' }}>
    {closedPeriod && <Alert type="warning" showIcon message="期间已结账，请先反结账后修改" />}
    {voucher.data?.sourceType && <Alert type="info" showIcon message="该凭证由业务模块生成，请在来源业务中修改" />}
    ...
    <Card><Form form={form} layout="vertical" initialValues={initial} disabled={!editable} onValuesChange={() => setDirty(true)} onFinish={(value) => save.mutate(value as VoucherForm)}>
      ...
      <Row gutter={16}>{voucherId !== 'new' && <Col xs={24} md={8}><Form.Item name="voucherNumber" label="凭证号" rules={[{ required: true }]}><Input /></Form.Item></Col>}<Col xs={24} md={voucherId === 'new' ? 24 : 16}><Form.Item name="summary" label="摘要"><Input /></Form.Item></Col></Row>
