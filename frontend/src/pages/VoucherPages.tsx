import { Alert, App as AntApp, Button, Card, Checkbox, DatePicker, Empty, Form, Input, Modal, Select, Space, Table, Tag, Typography, Upload } from 'antd'
...
const decimalRule = { pattern: voucherAmountPattern, message: '请输入有效金额，可使用负数' }
...
function VoucherAmountCell({ form, fieldName, side, onChange }: {
  ...
  return <Input className="voucher-amount-input" inputMode="decimal" value={visibleAmount}
    ...
    onChange={(event) => {
      form.setFieldValue(['lines', fieldName, 'side'], side)
      form.setFieldValue(['lines', fieldName, 'originalAmount'], event.target.value)
      onChange()
    }} />
}
...
export function VoucherEditorPage() {
  ...
  const selectedPeriodId = Form.useWatch('periodId', form)
  const selectedPeriod = periods.data?.find((period) => period.id === selectedPeriodId)
  const voucherPeriod = periods.data?.find((period) => period.id === voucher.data?.periodId)
  const closedPeriod = voucherId !== 'new' && voucherPeriod?.status === 'CLOSED'
  const editable = voucherId === 'new' || selectedPeriod?.status === 'OPEN'
  const save = useMutation({ mutationFn: async (value: VoucherForm) => { const existingVoucher = voucherId && voucherId !== 'new'; const body = { ...(existingVoucher ? { expectedVersion: voucher.data?.version, voucherNumber: value.voucherNumber } : {}), periodId: value.periodId, voucherDate: value.voucherDate.format('YYYY-MM-DD'), voucherType: value.voucherType, summary: value.summary, lines: value.lines.map(({ dimensionValues, ...line }) => ({ ...line, originalAmount: String(line.originalAmount), exchangeRate: String(line.exchangeRate), dimensions: Object.entries(dimensionValues || {}).filter(([, dimensionValueId]) => Boolean(dimensionValueId)).map(([dimensionTypeId, dimensionValueId]) => ({ dimensionTypeId, dimensionValueId })) })) }; return existingVoucher ? apiFetch<Voucher>(`/ledgers/${ledgerId}/vouchers/${voucherId}`, session!, { method: 'PUT', body: jsonBody(body) }) : apiFetch<Voucher>(`/ledgers/${ledgerId}/vouchers`, session!, { method: 'POST', headers: { 'Idempotency-Key': createIdempotencyKey() }, body: jsonBody(body) }) }, onSuccess: (value) => { setDirty(false); clearWorkspaceTabDirty(tabId); client.setQueryData(['voucher', ledgerId, value.id], value); void client.invalidateQueries({ queryKey: ['vouchers', ledgerId] }); if (voucherId === 'new') navigate(`/ledgers/${ledgerId}/vouchers/${value.id}`) }, onError: (error) => modal.error({ title: '凭证保存失败', content: error instanceof ApiError ? error.message : '请稍后重试。' }) })
  ...
  return <Space direction="vertical" size={16} style={{ width: '100%' }}>
      {closedPeriod && <Alert type="warning" showIcon message="期间已结账，不能修改或删除凭证" />}
    <div className="voucher-page-toolbar"><Typography.Text type="secondary">{voucher.data?.status || '新凭证'}</Typography.Text><Button onClick={() => navigate(`/ledgers/${ledgerId}/vouchers`)}>返回列表</Button></div>
    <Card className="voucher-editor-card"><Form className="voucher-editor-form" form={form} layout="vertical" initialValues={initial} disabled={!editable} onValuesChange={() => setDirty(true)} onFinish={(value) => save.mutate(value as VoucherForm)}>
      <div className="voucher-document-header">
        <div className="voucher-header-fields">
          <Form.Item name="voucherType" label="凭证字" rules={[{ required: true }]}><Input aria-label="凭证字" /></Form.Item>
          {voucherId !== 'new' ? <Form.Item name="voucherNumber" label="号" rules={[{ required: true }]}><Input aria-label="凭证号" /></Form.Item> : <span className="voucher-number-pending">号&nbsp;&nbsp;—</span>}
        </div>
        <div className="voucher-document-title">
          <Typography.Title level={1}>记账凭证</Typography.Title>
          <Form.Item name="periodId" rules={[{ required: true }]}><Select variant="borderless" aria-label="会计期间" placeholder="选择会计期间" options={(periods.data || []).map((period) => ({ value: period.id, label: `${period.periodCode}（${period.status}）` }))} /></Form.Item>
        </div>
        <div className="voucher-header-fields voucher-header-fields-right">
          <Form.Item name="voucherDate" label="日期" rules={[{ required: true }]}><DatePicker aria-label="凭证日期" /></Form.Item>
        </div>
      </div>
      <Form.List name="lines">{(fields, { add, remove }) => <>
        <Table className="voucher-entry-table" bordered size="middle" pagination={false} scroll={{ x: 900 }} rowKey="key" dataSource={fields}
          ...
          columns={[
            ...
            { title: '摘要', width: 240, render: (_, field) => <><Form.Item name={[field.name, 'side']} hidden rules={[{ required: true }]}><Input /></Form.Item><Form.Item name={[field.name, 'currency']} hidden><Input /></Form.Item><Form.Item name={[field.name, 'exchangeRate']} hidden><Input /></Form.Item><Form.Item name={[field.name, 'originalAmount']} hidden rules={[{ required: true }, decimalRule]}><Input /></Form.Item><Form.Item name={[field.name, 'summary']}><Input aria-label={`第 ${field.name + 1} 条分录摘要`} /></Form.Item></> },
            { title: '会计科目', width: 330, render: (_, field) => <Form.Item name={[field.name, 'accountId']} rules={[{ required: true }]}><Select showSearch optionFilterProp="label" aria-label={`第 ${field.name + 1} 条分录会计科目`} options={(accounts.data || []).filter((account) => account.status === 'ACTIVE' && account.isLeaf).map((account) => ({ value: account.id, label: `${account.code} ${account.name}` }))} /></Form.Item> },
