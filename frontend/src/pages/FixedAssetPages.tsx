...
export function FixedAssetEditorPage() {
  ...
  const save = useMutation({ mutationFn: (values: Record<string, unknown>) => apiFetch<FixedAsset>(`/ledgers/${ledgerId}/fixed-assets${assetId ? `/${assetId}` : ''}`, session!, { method: assetId ? 'PATCH' : 'POST', body: jsonBody(assetId ? { ...fixedAssetPayload(values, true), expectedVersion: asset.data?.version, changePeriodId: currentPeriod?.id, reason: '资产卡片维护' } : fixedAssetPayload(values, false)) }), onSuccess: (saved) => { void client.invalidateQueries({ queryKey: ['fixed-assets', ledgerId] }); messageApi.success('资产卡片已保存'); navigate(`/ledgers/${ledgerId}/fixed-assets/${saved.id}`) }, onError: (error) => messageApi.error(apiErrorMessage(error, '资产卡片保存失败')) })
