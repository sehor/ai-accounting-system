...
import type { Account, DimensionType, DimensionValue, FixedAsset, FixedAssetCategory, FixedAssetPage, Period } from '../api/types'
...
function useLedgerData(ledgerId: string) {
  ...
  const departmentValues = useQuery({
    ...
    queryFn: () => apiFetch<DimensionValue[]>(`/ledgers/${ledgerId}/dimension-types/${departmentType!.id}/values`, session!),
...
export function fixedAssetPayload(values: Record<string, unknown>, editing: boolean) {
  ...
  const normalized = {
    ...
    impairmentAmount: input.impairmentAmount ?? 0,
  }
  if (!editing) return normalized
...
export function FixedAssetEditorPage() {
  ...
  const save = useMutation({ mutationFn: (values: Record<string, unknown>) => apiFetch<FixedAsset>(`/ledgers/${ledgerId}/fixed-assets${assetId ? `/${assetId}` : ''}`, session!, { method: assetId ? 'PATCH' : 'POST', body: jsonBody(assetId ? { ...fixedAssetPayload(values, true), expectedVersion: asset.data?.version, effectivePeriodId: currentPeriod?.id, reason: '资产卡片维护' } : fixedAssetPayload(values, false)) }), onSuccess: (saved) => { void client.invalidateQueries({ queryKey: ['fixed-assets', ledgerId] }); messageApi.success('资产卡片已保存'); navigate(`/ledgers/${ledgerId}/fixed-assets/${saved.id}`) }, onError: (error) => messageApi.error(apiErrorMessage(error, '资产卡片保存失败')) })
  const copy = useMutation({ mutationFn: () => apiFetch<FixedAsset>(`/ledgers/${ledgerId}/fixed-assets/${assetId}:copy`, session!, { method: 'POST' }), onSuccess: (copied) => { messageApi.success('已创建副本'); void client.invalidateQueries({ queryKey: ['fixed-assets', ledgerId] }); navigate(`/ledgers/${ledgerId}/fixed-assets/${copied.id}`) }, onError: (error) => messageApi.error(apiErrorMessage(error, '复制资产卡片失败')) })
  const dispose = useMutation({ mutationFn: (values: DisposeFormValues) => apiFetch(`/ledgers/${ledgerId}/fixed-assets/${assetId}:dispose`, session!, { method: 'POST', body: jsonBody({ ...values, periodId: currentPeriod?.id, disposalDate: values.disposalDate?.format('YYYY-MM-DD'), proceeds: values.proceeds ?? 0, outputTax: values.outputTax ?? 0, clearingCost: values.clearingCost ?? 0, clearingInputTax: values.clearingInputTax ?? 0 }) }), onSuccess: () => { setDisposeOpen(false); messageApi.success('资产已清理并生成凭证'); void client.invalidateQueries({ queryKey: ['fixed-asset', ledgerId, assetId] }); void client.invalidateQueries({ queryKey: ['fixed-assets', ledgerId] }) }, onError: (error) => messageApi.error(apiErrorMessage(error, '资产清理失败')) })
  const remove = useMutation({ mutationFn: () => apiFetch<void>(`/ledgers/${ledgerId}/fixed-assets/${assetId}`, session!, { method: 'DELETE' }), onSuccess: () => { messageApi.success('资产卡片已删除'); void client.invalidateQueries({ queryKey: ['fixed-assets', ledgerId] }); navigate(`/ledgers/${ledgerId}/fixed-assets`) }, onError: (error) => messageApi.error(`${apiErrorMessage(error, '删除资产卡片失败')}；若资产已有历史，请先清理相关业务。`) })
