...
export function SettingsPage() {
  ...
  const openClosePanel = (period: Period) => {
    const closeDialog = modal.info({ title: `期末结账 · ${period.periodCode}`, width: 980, icon: null, footer: null, content: <PeriodClosingPanel ledgerId={ledgerId} session={session!} period={period} accounts={accounts.data || []} onClose={() => { closeDialog.destroy(); periodAction.mutate({ period, operation: 'close' }) }} /> })
  }
