...
export function SettingsPage() {
  ...
  const periodAction = useMutation({ mutationFn: ({ period, operation }: { period: Period; operation: 'close' | 'reopen' }) => apiFetch<Period>(`/ledgers/${ledgerId}/periods/${period.id}:${operation}`, session!, { method: 'POST', body: jsonBody({ reason: 'Manual period action' }) }), onSuccess: () => void client.invalidateQueries({ queryKey: ['periods', ledgerId] }) })
  const openingSave = useMutation({ mutationFn: (lines: OpeningFormLine[]) => apiFetch<OpeningBalance[]>(`/ledgers/${ledgerId}/opening-balances`, session!, { method: 'PUT', body: jsonBody({ lines: lines.map((line) => ({ ...line, dimensions: (line.dimensions || []).filter((dimension) => dimension.dimensionValueId).map((dimension) => ({ dimensionTypeId: dimension.dimensionTypeId, dimensionValueId: dimension.dimensionValueId })), debitOriginal: String(line.debitOriginal), creditOriginal: String(line.creditOriginal), exchangeRate: String(line.exchangeRate) })) }) }), onSuccess: () => { messageApi.success('期初余额已保存'); void client.invalidateQueries({ queryKey: ['openings', ledgerId] }) }, onError: (error) => messageApi.error(error instanceof ApiError ? error.message : '期初余额保存失败') })
...
export function DimensionsTab({ ledgerId, session, types, values, selectedTypeId, onSelect, onChanged }: { ledgerId: string; session: NonNullable<ReturnType<typeof useAuth>['session']>; types: DimensionType[]; values: DimensionValue[]; selectedTypeId?: string; onSelect: (id: string) => void; onChanged: () => void }) {
  ...
  const patchType = async (row: DimensionType, patch: { name?: string; status?: string }) => { setUpdating(row.id); try { await apiFetch<DimensionType>(`/ledgers/${ledgerId}/dimension-types/${row.id}`, session, { method: 'PATCH', body: jsonBody({ expectedVersion: row.version, ...patch }) }); onChanged() } finally { setUpdating(undefined) } }
  const patchValue = async (row: DimensionValue, patch: { name?: string; status?: string }) => { if (!selectedTypeId) return; setUpdating(row.id); try { await apiFetch<DimensionValue>(`/ledgers/${ledgerId}/dimension-types/${selectedTypeId}/values/${row.id}`, session, { method: 'PATCH', body: jsonBody({ expectedVersion: row.version, ...patch }) }); onChanged() } finally { setUpdating(undefined) } }
