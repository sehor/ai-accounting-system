...
export function PeriodClosingPanel({ ledgerId, session, period, accounts, onClose }: {
  ...
  const client = useQueryClient(); const [showSettings, setShowSettings] = useState(false)
  ...
  return <Space direction="vertical" style={{ width: '100%' }}>{contextHolder}
    ...
    {data && <>
      ...
      <Space wrap style={{ width: '100%' }}>{data.steps.map((step) => <StepCard key={step.step} step={step} onGenerate={() => generate.mutate(step.step)} onVoucher={() => window.location.assign(`/ledgers/${ledgerId}/vouchers/${step.voucherId}`)} loading={generate.isPending} />)}</Space>
