...
- 用户与账套多对多，账套是数据隔离边界。
- 首期实现小企业会计准则，多账套、多币种。
- 开放期间内的凭证可直接修改、物理删除和恢复历史版本；修改与删除同步更新余额投影。
- 重复单据只警告，不阻断。
- 默认不启用审批，可按账套开启。
...
| `voucher` | 凭证、校验、审批、记账、直接修改、物理删除、恢复 |
...
- `SUBMITTED → DRAFT`：退回。
- 开放期间内的凭证可直接修改；`POSTED` 凭证修改或恢复历史修订后仍保持 `POSTED`。
- v0.1 不实现反记账或冲销。
...
- 审批或退回。
- 记账及新增审计修订。
- 已记账凭证修改、删除或恢复时新增审计修订，并同时追加余额投影差额事件。
- 恢复历史版本。
...
- 只有 `CLOSED` 会计期间禁止修改和删除普通凭证。`OPEN` 期间内，手工和导入的普通凭证使用相同的修改、删除规则；`source_type/source_id` 非空的生成凭证由来源模块拥有，只能由来源模块修改或整体撤销。
- 恢复修订时复制目标快照，生成新的最大修订号，不回退计数器。
- 普通凭证删除先清理允许由该流程处理的业务引用，再物理删除凭证、分录、审批和幂等数据；删除前在同一事务内追加 `DELETE` 审计修订，已记账凭证同时追加余额投影冲减事件。`audit_revision` 只追加并永久保留，不随凭证删除。
- 恢复和关账后重开等高影响操作要求提供 `reason`。
...
- MCP 开放与 REST 相同的普通凭证修改、删除和记账能力，不暴露审批、关账、重新开账或成员管理；来源生成凭证仍只能由来源模块修改或整体撤销。
- 调用、参数哈希、结果哈希、操作者和 `traceId` 写入审计。

## 13. 幂等、并发与错误处理

- 创建凭证、上传文档、任务提交、记账和恢复支持幂等键。
- 幂等记录和业务写入在同一事务内完成。
- 可变聚合根使用 JPA `@Version`。
...
1. 领域单元测试：金额换算、借贷平衡、状态转换、恢复修订、修改和删除余额投影。
2. Repository 集成测试：约束、物理删除及引用清理、账套范围、任务领取并发。
...
5. 实现记账事务。
6. 实现历史修订列表与恢复。
7. 实现物理删除、外键引用清理和余额投影冲减。

验收：

- 人民币和外币凭证均能正确校验。
- 审批关闭和开启两条路径都可完成记账。
- 开放期间内的已记账凭证能直接修改并保持已记账状态。
- 恢复历史版本生成新修订，不覆盖历史。
- 物理删除已记账凭证后账簿净影响正确。
- 重复幂等请求只产生一个结果。
...
- Agent 只能访问其有权限的账套。
- 凭证修改、删除和记账工具均受期间规则约束，不存在审批、关账或成员管理工具。
