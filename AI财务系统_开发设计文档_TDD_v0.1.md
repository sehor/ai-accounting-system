...
- 只有 `CLOSED` 会计期间禁止修改和删除。`OPEN` 期间内，手工、自动生成和导入凭证使用相同的修改、删除规则。
- 恢复修订时复制目标快照，生成新的最大修订号，不回退计数器。
- 删除先清理固定资产等表对凭证和凭证行的引用，再物理删除凭证、分录、审批、幂等和审计数据；已记账凭证删除时在同一事务内追加余额投影冲减事件。
...
- MCP 开放与 REST 相同的凭证修改、删除和记账能力，不暴露审批、关账、重新开账或成员管理。
