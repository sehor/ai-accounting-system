...
账套编码规则由 `level2Width`、`level3Width`、`level4Width` 组成，默认 `2/2/2`，即 `4-2-2-2`。编码仅允许连续数字，不含点号或横杠；各子级段宽 1–8，总编码不超过 32 字符。

- 一级科目必须是四位数字，允许用户新增。
- 最大四级；子科目编码必须是父编码直接追加当前级定长数字段。
- 子科目继承父级类别与正常余额方向。
...
旧数据保留 UUID，科目编码迁移时去除 `.` 和 `-`，再按账套设置的各级段宽推断父级。无法推断的非四位根科目标记 `legacy_code=true`。遗留科目仍可记账和停用，但已使用遗留科目的核心属性锁定。
...
金蝶工作簿使用 `科目列表` 工作表，按金蝶原生四列格式导出：`编码`、`名称`、`类别`、`余额方向`；类别和余额方向使用中文值。
