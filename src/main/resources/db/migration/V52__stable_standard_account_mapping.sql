alter table ledger_account
    add column standard_account_key varchar(128);

alter table ledger_account
    add constraint ck_ledger_account_standard_account_key
        check (standard_account_key is null
            or standard_account_key ~ '^[A-Z][A-Z0-9]*(\.[A-Z0-9_]+)+$');

create index ix_ledger_account_ledger_standard_key
    on ledger_account (ledger_id, standard_account_key);

-- Only codes that are unique within the installed package are eligible.  The same
-- registry is declared in accounting-standards/*.json and is intentionally scoped
-- by standard/version because legacy codes are not globally stable.
with package_accounts(standard_code, standard_version, account_code, standard_key) as (
    values
    ...
    select account.id, account.ledger_id, min(package_accounts.standard_key) standard_key
    from ledger_account account
    join ledger on ledger.id = account.ledger_id
    join package_accounts
      on package_accounts.standard_code = upper(ledger.accounting_standard_code)
     and package_accounts.standard_version = case
     ...
     and package_accounts.account_code = account.code
    where account.is_template and account.standard_account_key is null
    group by account.id, account.ledger_id
    having count(distinct package_accounts.standard_key) = 1
)
update ledger_account account
set standard_account_key = exact_candidates.standard_key
from exact_candidates
where account.ledger_id = exact_candidates.ledger_id and account.id = exact_candidates.id;
...
begin
    ...
    if new.parent_id is null then
        ...
        if parent.standard_account_key is null
