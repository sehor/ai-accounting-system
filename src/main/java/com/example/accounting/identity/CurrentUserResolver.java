...
public class CurrentUserResolver {
        ...
        if (localHeaderEnabled) {
            ...
            if (header != null) {
                try {
                    ...
                    return new ResolvedUser(id, "local", header);
    ...
    private ResolvedUser resolveAuthenticatedUserDetails() {
        ...
        if (authentication instanceof JwtAuthenticationToken token) {
            ...
            return new ResolvedUser(UUID.nameUUIDFromBytes((issuer + "\u0000" + subject)
                    .getBytes(StandardCharsets.UTF_8)), issuer, subject);
        }
        if (authentication != null && authentication.isAuthenticated()) {
            try {
                UUID id = UUID.fromString(authentication.getName());
                return new ResolvedUser(id, "local", id.toString());
    ...
    public record ResolvedUser(UUID id, String issuer, String subject) {
    }
}
