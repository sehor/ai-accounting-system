...
MCP 的 `update_voucher` 和 `delete_voucher` 与 REST 使用相同规则：开放期间内，手工和导入凭证（包括已记账凭证）可通用修改或删除，并在同一事务中同步余额投影。带来源的自动生成凭证只能由其固定资产或期末结账等来源流程管理；通用修改、恢复或删除返回 HTTP 409，code 为 `GENERATED_VOUCHER_MANAGED_BY_SOURCE`。
